...
async function fetchProductInfo(
  productId: string
): Promise<{ id: string; name: string; image: string; price: number } | null> {
  try {
    ...
    if (res.ok) {
      ...
      const price =
        parseFloat(p.current_lowest_ask) ||
  ...
  return Promise.all(
    items.map(async (c: Record<string, unknown>) => {
      ...
      if (
        ...
        c.participants[0]?.avatar !== undefined
      ) {
      ...
      const fallback = (raw: unknown) => ({
        name: typeof raw === 'string' ? raw : 'User',
        username: 'user',
        avatar: undefined as string | undefined,
      });
      const [p1Info, p2Info] = await Promise.all([
        p1Id
          ? fetchUserInfo(p1Id, token)
          : Promise.resolve(fallback(p1NameRaw)),
        p2Id
          ? fetchUserInfo(p2Id, token)
          : Promise.resolve(fallback(p2NameRaw)),
      ]);
        ...
        participants: [
          {
            ...
            avatar: p1Info.avatar,
          },
          {
            id: p2Id,
            name: p2Info.name,
            username: p2Info.username,
            avatar: p2Info.avatar,
          },
        ],
        product,
        lastMessage: lastMsg
          ? {
              content:
                (lastMsg.original_text as string) ||
...
function normalizeMessage(m: Record<string, unknown>): ChatMessage {
  return {
    id: (m.id as string) || '',
    conversationId:
      (m.conversation_id as string) || (m.conversationId as string) || '',
    senderId: (m.sender_id as string) || (m.senderId as string) || '',
    content:
      (m.content as string) ||
      (m.body as string) ||
      (m.original_text as string) ||
      '',
