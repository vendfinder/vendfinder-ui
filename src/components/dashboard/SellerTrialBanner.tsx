...
export default function SellerTrialBanner() {
  ...
  return (
    <AnimatePresence>
      ...
      >
        <div
          className={`relative rounded-2xl overflow-hidden border ${isExpired ? 'border-red-500/30' : 'border-amber-500/30'}`}
        >
          <div
            className={`absolute inset-0 bg-gradient-to-br ${isExpired ? 'from-red-950/60 via-card to-red-950/30' : 'from-amber-950/40 via-card to-amber-950/20'}`}
          />
          ...
          <div className="relative p-6">
            <div className="flex flex-col sm:flex-row sm:items-center gap-4">
              <div className="flex items-start gap-4 flex-1">
                <div
                  className={`w-12 h-12 rounded-xl border flex items-center justify-center shrink-0 ${isExpired ? 'bg-red-500/15 border-red-500/30' : 'bg-amber-500/15 border-amber-500/30'}`}
                >
                  {isExpired ? (
                ...
                <div>
                  <h3 className="text-base font-bold text-foreground mb-1">
                    {isExpired
                      ? t('accessExpired')
                      : t('renewalSoon', { days: daysRemaining })}
                  </h3>
                  <p className="text-sm text-muted leading-relaxed">
                    {isExpired
                      ? t('payToRestore', { fee: '$100' })
                      : t('renewToKeepAccess')}
                  </p>
                </div>
              </div>
              ...
              >
                {redirecting ? (
                  ...
                  <>
                    <CreditCard size={16} />
                    {isExpired ? t('activateAccount') : t('renewNow')}
                  </>
                )}
              </button>
            </div>
