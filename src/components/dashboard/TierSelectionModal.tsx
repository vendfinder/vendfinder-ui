        ...
        <div className="flex items-center justify-between p-5 border-b border-border sticky top-0 bg-card z-10">
          <div>
            ...
            <p className="text-[12px] text-muted mt-0.5">
              Billed every 30 days. Cancel or change anytime.
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            {TIERS.map((tier) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`w-9 h-9 rounded-xl ${tier.bgColor} ${tier.color} flex items-center justify-center mb-3`}
                  >
                    <Icon size={18} />
                  </div>
                  <p
                    className={`text-sm font-bold ${isSelected ? tier.color : 'text-foreground'}`}
                  >
                    {tier.label}
                  </p>
                  ...
                  <ul className="space-y-1.5 flex-1">
                    {tier.features.map((feature) => (
                      ...
                      >
                        <Check
                          size={12}
                          className={`${tier.color} shrink-0 mt-0.5`}
                        />
                        <span>{feature}</span>
                      </li>
                    ))}
