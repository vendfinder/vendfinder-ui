...
const sellerTerms = [
  { title: "1. Seller Account & Eligibility", body: "You must be at least 18 years old and provide accurate business information. Seller accounts are subject to approval by VendFinder. You are responsible for maintaining the accuracy of your vendor profile, including business name, contact information, and product listings." },
  { title: "2. Platform Fees", body: "VendFinder charges a 10% platform fee on each completed sale. This fee is automatically deducted from your earnings before payout. The fee covers payment processing, platform maintenance, customer support, and dispute resolution services." },
  { title: "3. Payouts", body: "Seller earnings are available for payout after an order is marked as delivered and the buyer confirmation period has passed. Payouts are processed on a regular schedule to your designated payment method. VendFinder may hold payouts if there are pending disputes or suspected policy violations." },
  { title: "4. Product Listings & Obligations", body: "You are responsible for the accuracy of your product listings, including descriptions, pricing, images, and stock levels. You must fulfill orders promptly and maintain reasonable shipping timelines. You may not list prohibited items including counterfeit goods, illegal substances, or items that violate intellectual property rights." },
  { title: "5. Disputes & Returns", body: "You must respond to buyer disputes within 48 hours. VendFinder may mediate disputes and issue refunds on your behalf if you fail to respond or if VendFinder determines the buyer's claim is valid. Excessive disputes or poor seller ratings may result in account review or suspension." },
  { title: "6. Seller Conduct", body: "You may not engage in price manipulation, fake reviews, shill bidding, or any deceptive practices. You must comply with all applicable laws and regulations for your business. VendFinder reserves the right to suspend or terminate seller accounts that violate these terms." },
  { title: "7. Limitation of Liability", body: "VendFinder provides the marketplace platform but is not a party to transactions between buyers and sellers. You are solely responsible for the products you sell, including product safety, legal compliance, and customer satisfaction. VendFinder is not liable for losses arising from your business operations on the platform." },
  { title: "8. Changes to Terms", body: "VendFinder may update these terms at any time. You will be notified of material changes and asked to re-accept the updated terms. Continued use of the platform after notification constitutes acceptance of the updated terms." },
];
...
export default function PublicProfilePage() {
  ...
  const { favorites, portfolio, sellerStats: ownStats, listings } = useDashboardData();
  const isOwnProfile = user?.username === params.username;
  const [publicStats, setPublicStats] = useState<{ totalSales: number; avgShipTime: string; completionRate: number; sellerRating: number; verification?: { verified: boolean; proSeller: boolean; topRated: boolean; kycVerified: boolean } } | null>(null);

  // For own profile, use full stats from dashboard; for others, use public stats
  const sellerStats = isOwnProfile ? ownStats : {
  ...
  useEffect(() => {
    ...
    fetchUserStories(profileUser.id, token || undefined).then(setProfileStories);
  ...
  useEffect(() => {
    ...
    fetch(`/api/reviews?vendor_id=${profileUser.id}&limit=50`)
      ...
      .then(data => {
        const items: FeedbackItem[] = (data.reviews || []).map((r: Record<string, unknown>) => ({
          id: r.id as string,
          buyer: (r.author as { username?: string })?.username || (r.author_name as string) || "Anonymous",
  ...
  const tabs: { key: ProfileTab; label: string; count: number; icon: React.ReactNode }[] = [
    { key: "selling", label: t("profile.selling"), count: activeListings.length, icon: <Tag size={13} /> },
    { key: "feedback", label: t("profile.feedback"), count: feedbackItems.length, icon: <Star size={13} /> },
    { key: "favorites", label: t("profile.favorites"), count: favorites.length, icon: <Heart size={13} /> },
    { key: "collection", label: t("profile.collection"), count: portfolio.length, icon: <Briefcase size={13} /> },
    { key: "terms", label: t("profile.sellerTerms"), count: sellerTerms.length, icon: <FileText size={13} /> },
  ];
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="relative h-44 sm:h-56 overflow-hidden">
        {displayUser?.banner ? (
          <img src={displayUser.banner} alt="" className="absolute inset-0 w-full h-full object-cover" />
        ) : (
          <>
            {/* Gradient base */}
      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex flex-col sm:flex-row sm:items-end gap-5 sm:gap-6">
            ...
            >
                ...
                <div
                  ...
                  style={{
                    background: hasStories
                      ...
                      : "transparent",
                    animation: hasStories && hasUnviewedStories
                  ...
                  <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-[13px] bg-card shadow-2xl shadow-black/30 flex items-center justify-center text-primary font-bold text-4xl sm:text-5xl overflow-hidden">
                    {displayUser?.avatar ? (
                      ...
                      <>
                        ...
                        <span className="relative">{displayUser?.name?.charAt(0) || "A"}</span>
                      </>
                    )}
                  </div>
                </div>
              ...
              <div className="absolute -top-2 -right-2 w-9 h-9 bg-card border-2 border-primary/60 rounded-xl flex items-center justify-center shadow-lg z-10">
                <span className="text-primary font-black text-xs">Lv{displayUser?.sellerLevel || 1}</span>
              </div>
            </motion.div>
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2.5 flex-wrap">
                    ...
                    <VerificationBadge
                      verified={publicStats?.verification?.verified ?? displayUser?.verified}
                  ...
                  <p className="text-muted text-sm mt-0.5">@{displayUser?.username || params.username}</p>
                  {displayUser?.bio && (
                    <p className="text-foreground/70 text-sm mt-2.5 max-w-md leading-relaxed">{displayUser?.bio}</p>
                  )}
                  ...
                  {isOwnProfile ? (
                    ...
                    <>
                      ...
                      {isAuthenticated && profileUser?.id && (
                        <button
                          onClick={() => router.push(`/dashboard/messages?seller=${profileUser.id}`)}
        ...
        >
          {[
            { key: "followers", label: t("profile.followers"), value: displayUser?.followers?.toLocaleString() || "0", icon: Users, color: "text-blue-400", bgColor: "bg-blue-400/10", borderColor: "border-blue-400/15" },
            { key: "following", label: t("profile.followingCount"), value: (displayUser?.following || 0).toString(), icon: UserPlus, color: "text-violet-400", bgColor: "bg-violet-400/10", borderColor: "border-violet-400/15" },
            { key: "totalSales", label: t("profile.totalSales"), value: sellerStats.totalSales.toString(), icon: Package, color: "text-emerald-400", bgColor: "bg-emerald-400/10", borderColor: "border-emerald-400/15" },
            { key: "rating", label: t("profile.rating"), value: sellerStats.totalSales > 0 ? sellerStats.sellerRating.toString() : "—", icon: Star, color: "text-amber-400", bgColor: "bg-amber-400/10", borderColor: "border-amber-400/15", isStar: true },
            { key: "avgShip", label: t("profile.avgShip"), value: sellerStats.avgShipTime, icon: Clock, color: "text-primary", bgColor: "bg-primary/10", borderColor: "border-primary/15" },
          ].map((stat, i) => {
        ...
        >
          ...
          <div className="relative flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-sm font-bold text-foreground">{t("profile.sellerLevel", { level: displayUser?.sellerLevel || 1 })}</p>
            ...
            <div className="flex items-center gap-6 text-[12px]">
              <span className="flex items-center gap-1.5 text-muted">
                ...
                <span className="font-semibold text-foreground">{sellerStats.avgShipTime}</span> {t("profile.avgShipLabel")}
              </span>
              <span className="w-px h-4 bg-white/[0.06]" />
              <span className="flex items-center gap-1.5 text-muted">
                <CheckCircle2 size={12} className="text-emerald-400" />
                <span className="font-semibold text-foreground">{sellerStats.completionRate}%</span> {t("profile.completion")}
              </span>
              <span className="w-px h-4 bg-white/[0.06]" />
              <span className="flex items-center gap-1.5 text-muted">
                <Star size={12} className="text-amber-400 fill-amber-400" />
                <span className="font-semibold text-foreground">{sellerStats.totalSales > 0 ? sellerStats.sellerRating : "—"}</span> {t("profile.ratingLabel")}
              </span>
            </div>
          </div>
        </motion.div>
              ...
              >
                {activeListings.length === 0 ? (
                  <div className="bg-card rounded-2xl border border-border py-16 text-center">
                    ...
                    <p className="text-foreground font-medium">{t("profile.noActiveListings")}</p>
                    <p className="text-sm text-muted mt-1">{t("profile.noActiveListingsDesc")}</p>
                  </div>
                ) : (
                      ...
                      >
                        <div className="aspect-square bg-surface relative flex items-center justify-center text-muted/20 overflow-hidden">
                          {listing.productImage ? (
                            <img src={listing.productImage} alt={listing.productName} className="absolute inset-0 w-full h-full object-cover" />
                          ) : (
                            <Package size={48} />
                          )}
                          {/* Condition badge */}
                          <div className="absolute top-3 right-3">
                            <span className="text-[9px] font-bold uppercase tracking-wider px-2 py-1 rounded-lg bg-black/40 backdrop-blur-sm text-white border border-white/10">
                              {listing.condition === "new" ? t("profile.conditionNew") : t("profile.conditionUsed")}
                            </span>
                          </div>
                          {/* Views */}
                        ...
                        <div className="p-4">
                          ...
                          <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/[0.04]">
                            <div>
                              <p className="text-[10px] text-muted uppercase tracking-wider font-semibold">{t("profile.ask")}</p>
                              <p className="text-lg font-bold text-primary">{formatPrice(listing.askPrice)}</p>
                            </div>
                            {listing.lastSale && (
                              <div className="text-right">
                                <p className="text-[10px] text-muted uppercase tracking-wider font-semibold">{t("profile.lastSale")}</p>
              ...
              >
                {feedbackItems.length === 0 ? (
                  <div className="bg-card rounded-2xl border border-border py-16 text-center">
                    ...
                    <p className="text-foreground font-medium">{t("profile.noFeedback")}</p>
                    <p className="text-sm text-muted mt-1">{t("profile.noFeedbackDesc")}</p>
                  </div>
                ) : (
                  feedbackItems.map((item, i) => (
                    ...
                    >
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <div>
                          <span className="text-sm font-semibold text-foreground">{item.buyer}</span>
                          <div className="flex gap-0.5 mt-0.5">
                            {Array.from({ length: 5 }).map((_, j) => (
                              <Star
                                key={j}
                                size={10}
                                className={j < item.rating ? "fill-amber-400 text-amber-400" : "text-white/[0.06]"}
                              />
                            ))}
                          </div>
                        </div>
                      ...
                      <p className="text-sm text-muted mt-1 leading-relaxed">{item.comment}</p>
                      <p className="text-[11px] text-muted/50 mt-2">{item.date}</p>
                    </motion.div>
                  ))
                )}
              </motion.div>
                    ...
                    >
                      ...
                      <div className="p-4">
                        ...
                        <div className="flex items-center justify-between mt-3 pt-3 border-t border-white/[0.04]">
                          <div>
                            <p className="text-[10px] text-muted uppercase tracking-wider font-semibold">{t("profile.lowestAsk")}</p>
                ...
                <div className="bg-card rounded-2xl border border-violet-400/15 p-5 mb-6 relative overflow-hidden">
                  ...
                  <div className="relative flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <p className="text-[10px] text-muted uppercase tracking-wider font-semibold">{t("profile.collectionValue")}</p>
                    ...
                    <div className="text-right">
                      <p className="text-[10px] text-muted uppercase tracking-wider font-semibold">{t("profile.items")}</p>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="p-4">
                          ...
                          <div className="flex items-center justify-between mt-4 pt-3 border-t border-white/[0.04]">
                            <div>
                              <p className="text-[10px] text-muted uppercase tracking-wider font-semibold">{t("profile.value")}</p>
                              <p className="text-sm font-bold text-foreground">{formatPrice(item.currentValue)}</p>
                            </div>
                            <div className="text-right">
                              <p className="text-[10px] text-muted uppercase tracking-wider font-semibold">{t("profile.gainLoss")}</p>
                ...
                <div className="bg-card rounded-2xl border border-border overflow-hidden max-w-3xl">
                  <div className="px-6 py-5 border-b border-white/[0.04]">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <h3 className="text-base font-bold text-foreground">{t("profile.sellerTermsTitle")}</h3>
                        <p className="text-[11px] text-muted mt-0.5">{t("profile.sellerTermsUpdated")}</p>
                      </div>
                    </div>
                  </div>
