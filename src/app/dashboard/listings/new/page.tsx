...
import { createProduct, createAsk, uploadProductImages } from "@/lib/api-products";
...
export default function NewListingPage() {
  ...
  const conditions = [
    { value: "new", label: t("conditionNew"), description: t("conditionNewDesc"), icon: Sparkles, color: "text-emerald-400", bgColor: "bg-emerald-400/10" },
    { value: "used_like_new", label: t("conditionLikeNew"), description: t("conditionLikeNewDesc"), icon: ShieldCheck, color: "text-blue-400", bgColor: "bg-blue-400/10" },
    { value: "used_good", label: t("conditionGood"), description: t("conditionGoodDesc"), icon: CheckCircle2, color: "text-amber-400", bgColor: "bg-amber-400/10" },
    { value: "used_fair", label: t("conditionFair"), description: t("conditionFairDesc"), icon: Info, color: "text-muted", bgColor: "bg-surface" },
  ];
  ...
  const showSizes = formData.category === "sneakers" || formData.category === "apparel";
            ...
            >
              ...
              <div className="bg-card rounded-2xl border border-border p-5">
                <div className="flex items-center gap-2.5 mb-5">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-foreground">{t("productDetails")}</p>
                    <p className="text-[11px] text-muted">{t("productDetailsDesc")}</p>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    <label className="text-[11px] text-muted font-semibold uppercase tracking-wider mb-1.5 block">
                      {t("description")} <span className="text-muted/40 normal-case">({t("optional")})</span>
                    </label>
                    <textarea
              ...
              <div className="bg-card rounded-2xl border border-border p-5">
                <div className="flex items-center gap-2.5 mb-5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-muted">{t("categoryDesc")}</p>
                  </div>
                </div>
                  ...
                  {categories.map((cat) => {
                    ...
                    return (
                      <button
                        key={cat.slug}
                        type="button"
                        onClick={() => setFormData({ ...formData, category: cat.slug, sizes: [] })}
                      ...
                      >
                        ...
                        <p className="text-[10px] text-muted mt-0.5">{cat.description}</p>
                      </button>
                    );
                  })}
                </div>
              ...
              <div className="bg-card rounded-2xl border border-border p-5">
                <div className="flex items-center gap-2.5 mb-5">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-muted">{t("photosDesc")}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-4 gap-3">
                  ...
                  {previews.map((src, i) => (
                    <div key={i} className="aspect-square rounded-xl border border-border bg-surface relative overflow-hidden group">
                      <img src={formData.images[i] || src} alt={`Photo ${i + 1}`} className="w-full h-full object-cover" />
                      {/* Uploading overlay */}
                  ...
                  {formData.images.length < 8 && (
                    ...
                    >
                      ...
                      <span className="text-[9px] font-bold uppercase tracking-wider">{t("addPhoto")}</span>
                    </button>
                  )}
            ...
            >
              ...
              <div className="bg-card rounded-2xl border border-border p-5">
                <div className="flex items-center gap-2.5 mb-5">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-foreground">{t("conditionTitle")}</p>
                    <p className="text-[11px] text-muted">{t("conditionDesc")}</p>
                  </div>
                </div>
              ...
              {showSizes && (
                <div className="bg-card rounded-2xl border border-border p-5">
                  <div className="flex items-center gap-2.5 mb-5">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center justify-between">
                        <p className="text-sm font-semibold text-foreground">{t("sizeTitle")}</p>
                      ...
                      <p className="text-[11px] text-muted">{t("sizeDesc")}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    <button
                      type="button"
                      onClick={() => setFormData({ ...formData, sizes: [...sizeOptions] })}
            ...
            >
              ...
              <div className="bg-card rounded-2xl border border-border p-5">
                <div className="flex items-center gap-2.5 mb-5">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-foreground">{t("setYourAskPrice")}</p>
                    <p className="text-[11px] text-muted">{t("setYourAskPriceDesc")}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[
                    { label: t("marketLowestAsk"), value: "$—", color: "text-emerald-400" },
                    { label: t("marketHighestBid"), value: "$—", color: "text-blue-400" },
                    { label: t("marketLastSale"), value: "$—", color: "text-foreground" },
                  ].map((item) => (
                  ...
                  >
                    <div className="bg-card rounded-2xl border border-emerald-400/15 p-5">
                      <div className="flex items-center gap-2.5 mb-4">
                        ...
                        <div>
                          <p className="text-sm font-semibold text-foreground">{t("payoutSummary")}</p>
                          <p className="text-[11px] text-muted">{t("payoutSummaryDesc")}</p>
                        </div>
                      </div>

                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-muted">{t("salePrice")}</span>
                          <span className="text-sm font-semibold text-foreground">${askNum.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-muted">{t("sellerFee")}</span>
                          <span className="text-sm font-semibold text-red-400">-${sellerFee.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-muted">{t("paymentProcessing")}</span>
                          <span className="text-sm font-semibold text-red-400">-${processingFee.toFixed(2)}</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-muted">{t("globalListingFeeLabel")}</span>
                          <span className="text-sm font-semibold text-red-400">-${globalFee.toFixed(2)}</span>
                        </div>
                        <div className="border-t border-white/[0.06] pt-3 mt-1">
                          <div className="flex justify-between items-center">
                            <span className="text-sm font-bold text-foreground">{t("totalPayout")}</span>
                          ...
                          <p className="text-[10px] text-muted mt-1.5">{t("youKeepPercent")}</p>
                        </div>
                      </div>
                    </div>
                  </motion.div>
              ...
              <div className="bg-card rounded-2xl border border-border p-5">
                <div className="flex items-center gap-2.5 mb-5">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-foreground">{t("listingDuration")}</p>
                    <p className="text-[11px] text-muted">{t("listingDurationDesc")}</p>
                  </div>
                </div>
              ...
              <div className="bg-card rounded-2xl border border-primary/20 p-5 relative overflow-hidden">
                ...
                <div className="relative">
                  <p className="text-[10px] text-muted/60 uppercase tracking-[0.12em] font-bold mb-3">{t("listingPreview")}</p>
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-1.5">
                        ...
                        {formData.sizes.length > 0 && (
                          <span className="text-[10px] bg-surface px-2 py-0.5 rounded-lg text-muted border border-border">
                            {formData.sizes.length === 1 ? formData.sizes[0] : `${formData.sizes.length} sizes`}
                          </span>
                        )}
                        <span className="text-[10px] bg-surface px-2 py-0.5 rounded-lg text-muted capitalize border border-border">
