...
export default function SellingPage() {
  ...
  const [featuringListing, setFeaturingListing] = useState<{
    productId: string;
    productName: string;
  } | null>(null);
  const [sponsoringListing, setSponsoringListing] = useState<{
    productId: string;
    productName: string;
    category: string;
  } | null>(null);
  ...
  const tabs: {
    ...
    },
    {
      key: 'orders',
      label: t('orders'),
      count: activeSellerOrders.length,
          ...
          {stats.map((stat, i) => {
            ...
            return (
              ...
              >
                ...
                <p
                  className={`text-2xl font-bold tracking-tight ${stat.label === t('totalRevenue') ? 'text-emerald-400' : 'text-foreground'}`}
                >
                  {stat.value}
                </p>
              </motion.div>
            ...
            <div className="divide-y divide-white/[0.04]">
              {activeSellerOrders.length === 0 ? (
                ...
                activeSellerOrders.map((sale, i) => {
                  const config =
                    saleOrderStatusConfig[sale.status] ||
                    saleOrderStatusConfig.processing;
                  return (
                    <motion.div
                      key={sale.id}
                    ...
                    >
                            ...
                            sale.shipping_name && (
                              <p className="text-[10px] text-muted mt-1 truncate">
                                ...
                                <span className="text-foreground/70">
                                  {sale.shipping_name}, {sale.shipping_city},{' '}
                                  {sale.shipping_state}
                                </span>
                              </p>
                            )}
                      ...
                      <div className="text-right">
                        <p className="text-sm font-bold text-foreground">
                          {formatPrice(parseFloat(String(sale.item_price)))}
                        </p>
                      </div>
                      ...
                      <div className="text-right">
                        <p className="text-sm font-semibold text-emerald-400">
                          {formatPrice(parseFloat(String(sale.seller_payout)))}
                        </p>
                        <span className="text-[9px] text-muted block mt-0.5">
                          -{formatPrice(parseFloat(String(sale.platform_fee)))}{' '}
                          {t('fee')}
                        </span>
                      </div>
              ...
              <div className="flex items-center justify-between px-5 py-3 border-t border-white/[0.04] bg-white/[0.01]">
                ...
                <span className="text-[11px] text-muted">
                  ...
                  <span className="font-semibold text-emerald-400">
                    {formatPrice(
                      activeSellerOrders.reduce(
                        (sum, s) => sum + parseFloat(String(s.seller_payout)),
                        0
                      )
                    )}
                  </span>
          ...
          {shippingOrder && (
            ...
            >
              <div
                ...
                onClick={() => !shipping && setShippingOrder(null)}
              />
              <motion.div
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-1.5 mt-0.5">
                        ...
                        <span className="text-[11px] text-muted">
                          {formatPrice(
                            parseFloat(String(shippingOrder.item_price))
                          )}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="bg-surface/60 rounded-xl border border-border/60 p-4">
                      ...
                      <div className="text-sm text-foreground space-y-0.5">
                        ...
                        <p className="text-muted">
                          {shippingOrder.shipping_address_line1}
                        </p>
                        ...
                        <p className="text-muted">
                          {shippingOrder.shipping_city},{' '}
                          {shippingOrder.shipping_state}{' '}
                          {shippingOrder.shipping_zip}
                        </p>
                        {shippingOrder.shipping_country &&
                          shippingOrder.shipping_country !== 'US' && (
                            <p className="text-muted">
                              {shippingOrder.shipping_country}
                            </p>
                          )}
              ...
              <AnimatePresence mode="popLayout">
                {filteredListings.length === 0 ? (
                  ...
                  >
                    ...
                    <p className="text-foreground font-medium">
                      {t('noListings', { tab: activeTab })}
                    </p>
                    <p className="text-sm text-muted mt-1 mb-4">
                      {t('noListingsDescription', { tab: activeTab })}
                    </p>
                    {activeTab === 'active' && (
                      <Link
                        href="/dashboard/listings/new"
                  ...
                  filteredListings.map((listing, i) => {
                    ...
                    const isAboveLowest =
                      listing.lowestAsk && listing.askPrice > listing.lowestAsk;
                    const isBelowLowest =
                      listing.lowestAsk &&
                      listing.askPrice <= listing.lowestAsk;
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="text-right hidden sm:block">
                          <p
                            className={`text-sm ${listing.lowestAsk ? 'text-foreground/80' : 'text-muted/30'}`}
                          >
                            {listing.lowestAsk
                              ? formatPrice(listing.lowestAsk)
                              : '\u2014'}
                          </p>
                        </div>
                        ...
                        <div className="text-right hidden sm:block">
                          <p
                            className={`text-sm ${listing.highestBid ? 'text-foreground/80' : 'text-muted/30'}`}
                          >
                            {listing.highestBid
                              ? formatPrice(listing.highestBid)
                              : '\u2014'}
                          </p>
                        </div>
                        ...
                        <div className="text-right hidden sm:block">
                          <p
                            className={`text-sm ${listing.lastSale ? 'text-foreground/80' : 'text-muted/30'}`}
                          >
                            {listing.lastSale
                          ...
                          <AnimatePresence>
                            {openMenu === listing.id && (
                              ...
                              >
                                <button
                                  onClick={() =>
                                    handleEditAsk(listing.id, listing.askPrice)
                                  }
                                  className="flex items-center gap-2 w-full px-3 py-2.5 text-xs font-medium text-foreground hover:bg-surface transition-colors"
                                >
              ...
              <div className="flex items-center justify-between px-5 py-3 border-t border-white/[0.04] bg-white/[0.01]">
                ...
                <div className="flex items-center gap-4 text-[11px] text-muted">
                  <span>
                    ...
                    <span className="font-semibold text-foreground">
                      {formatPrice(
                        filteredListings.reduce((sum, l) => sum + l.askPrice, 0)
                      )}
                    </span>
                  </span>
                  <span>
