...
export default function BuyingPage() {
  ...
  const { token } = useAuth();
  const { bids, purchases, orders, sellerStats, loading, refetch } = useDashboardData();

  const purchaseStatusConfig: Record<
    string,
    { variant: "info" | "warning" | "success" | "error"; icon: React.ReactNode; label: string }
  > = {
    pending_shipment: { variant: "warning", icon: <Clock size={10} />, label: t("statusAwaitingShipment") },
    shipped: { variant: "info", icon: <Truck size={10} />, label: t("statusInTransit") },
    delivered: { variant: "success", icon: <CheckCircle2 size={10} />, label: t("statusDelivered") },
    authenticated: { variant: "success", icon: <CheckCircle2 size={10} />, label: t("statusAuthenticated") },
    cancelled: { variant: "error", icon: <XCircle size={10} />, label: t("statusCancelled") },
  };
  ...
  > = {
    processing: { variant: "info", icon: <Clock size={10} />, label: t("statusProcessing"), bgColor: "bg-blue-400" },
  ...
  const tabs: { key: Tab; label: string; count: number; icon: React.ReactNode }[] = [
    { key: "bids", label: t("activeBids"), count: activeBids.length, icon: <Gavel size={13} /> },
    { key: "orders", label: t("orders"), count: activeOrders.length, icon: <ShoppingBag size={13} /> },
    { key: "pending", label: t("pending"), count: pendingPurchases.length, icon: <Clock size={13} /> },
    { key: "history", label: t("history"), count: completedPurchases.length, icon: <CheckCircle2 size={13} /> },
  ];
          ...
          {activeTab === "bids" && (
            <motion.div key="bids" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="hidden sm:grid grid-cols-[1fr_95px_95px_95px_95px_50px] gap-3 px-5 py-3 border-b border-white/[0.04] bg-white/[0.01]">
                ...
                <span className="text-[10px] text-muted/60 uppercase tracking-[0.12em] font-bold text-right">{t("headerYourBid")}</span>
              ...
              <div className="divide-y divide-white/[0.04]">
                {activeBids.length === 0 ? (
                  <div className="py-16 text-center">
                    ...
                    <p className="text-foreground font-medium">{t("noActiveBids")}</p>
                    <p className="text-sm text-muted mt-1 mb-4">{t("noActiveBidsDescription")}</p>
                    <Link
                      href="/products"
              ...
              {activeBids.length > 0 && (
                <div className="flex items-center justify-between px-5 py-3 border-t border-white/[0.04] bg-white/[0.01]">
                  <p className="text-[11px] text-muted">
                    <span className="font-semibold text-foreground">{activeBids.length}</span> {activeBids.length !== 1 ? t("activeBidsPlural") : t("activeBidSingular")}
                  </p>
          ...
          {activeTab === "orders" && (
            <motion.div key="orders" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              ...
              <div className="hidden sm:grid grid-cols-[1fr_100px_100px_50px] gap-3 px-5 py-3 border-b border-white/[0.04] bg-white/[0.01]">
                ...
                <span className="text-[10px] text-muted/60 uppercase tracking-[0.12em] font-bold text-right">{t("headerTotal")}</span>
              ...
              <div className="divide-y divide-white/[0.04]">
                {activeOrders.length === 0 ? (
                  <div className="py-16 text-center">
                    ...
                    <p className="text-foreground font-medium">{t("noOrdersYet")}</p>
                    <p className="text-sm text-muted mt-1 mb-4">{t("noOrdersDescription")}</p>
                    <Link
                      href="/products"
                      className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-primary/10 text-primary text-sm font-medium hover:bg-primary/15 transition-colors"
                    >
                  ...
                  activeOrders.map((order, i) => {
                    ...
                    const trackingNumber = order.tracking_number as string | undefined;
                    const carrier = order.carrier as string | undefined;
                    const productName = (order.product_name as string) || "";
                    const total = parseFloat(String(order.total_buyer_pays || 0));
                    const orderDate = order.created_at ? new Date(order.created_at as string).toLocaleDateString() : "";
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          <div className="w-12 h-12 rounded-xl bg-surface border border-border flex items-center justify-center text-muted/30 shrink-0 group-hover:border-border-hover transition-colors relative overflow-hidden">
                            {order.product_image ? (
                              <Image src={order.product_image as string} alt={productName} fill className="object-cover" sizes="48px" />
                            ) : (
                              <Package size={18} />
                            )}
                            <div className={`absolute -top-1 -right-1 w-3 h-3 rounded-full border-2 border-card ${config.bgColor} z-10`} />
                          </div>
                          <div className="min-w-0">
                            <p className="text-sm font-medium text-foreground truncate group-hover:text-primary transition-colors">
                              {productName}
                            ...
                            <div className="flex items-center gap-1.5 mt-0.5">
                              <span className="text-[11px] text-muted">{order.order_number as string} &middot; {orderDate}</span>
                            </div>
                            {trackingNumber && (() => {
                        ...
                        <div className="text-right relative">
                          <button
                            onClick={() => setOpenMenu(openMenu === orderId ? null : orderId)}
                            className="p-1.5 rounded-lg hover:bg-surface text-muted/50 hover:text-foreground transition-colors opacity-0 group-hover:opacity-100"
                          >
                            <MoreHorizontal size={16} />
                          </button>
                          <AnimatePresence>
                            {openMenu === orderId && (
                              ...
                              >
                                <button
                                  onClick={() => { setDetailOrder(order); setOpenMenu(null); }}
                                ...
                                {["processing", "pending_payment"].includes(status) && (
                                  <>
                                    ...
                                    >
                                      <XCircle size={12} />
                                      {cancelling === orderId ? t("cancellingOrder") : t("cancelOrder")}
                                    </button>
                                  </>
                                )}
                              </motion.div>
              ...
              {activeOrders.length > 0 && (
                <div className="flex items-center justify-between px-5 py-3 border-t border-white/[0.04] bg-white/[0.01]">
                  <p className="text-[11px] text-muted">
                    <span className="font-semibold text-foreground">{activeOrders.length}</span> {activeOrders.length !== 1 ? t("ordersPlural") : t("orderSingular")}
                  </p>
                  <span className="text-[11px] text-muted">
                    {t("total")}:{" "}
                    <span className="font-semibold text-foreground">
                      {formatPrice(activeOrders.reduce((sum, o) => sum + parseFloat(String(o.total_buyer_pays || 0)), 0))}
                    </span>
                  </span>
                </div>
              )}
              ...
              <div className="divide-y divide-white/[0.04]">
                {pendingPurchases.length === 0 ? (
                  <div className="py-16 text-center">
                    ...
                    <p className="text-foreground font-medium">{t("noPendingPurchases")}</p>
                    <p className="text-sm text-muted mt-1">{t("noPendingDescription")}</p>
                  </div>
                ) : (
                      ...
                      >
                        <div className="w-14 h-14 rounded-xl bg-surface border border-border flex items-center justify-center text-muted/30 shrink-0 group-hover:border-border-hover transition-colors relative overflow-hidden">
                          {purchase.productImage ? (
                            <Image src={purchase.productImage} alt={purchase.productName} fill className="object-cover" sizes="56px" />
                          ) : (
                            <Package size={22} />
                          )}
                          {/* Status dot */}
                          <div className={`absolute -top-1 -right-1 w-3 h-3 rounded-full border-2 border-card z-10 ${
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className="text-[11px] text-muted mt-0.5">
                            {purchase.size && `${purchase.size} \u00b7 `}{t("from")}{" "}
                        ...
                        <div className="text-right shrink-0 flex flex-col items-end gap-1.5">
                          ...
                          {purchase.status === "pending_shipment" && (
                            ...
                            >
                              {cancelling === purchase.id ? t("cancellingOrder") : t("cancelOrder")}
                            </button>
                          )}
                        </div>
                      </motion.div>
          ...
          {activeTab === "history" && (
            <motion.div key="history" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <div className="divide-y divide-white/[0.04]">
                {completedPurchases.length === 0 ? (
                  <div className="py-16 text-center">
                    ...
                    <p className="text-foreground font-medium">{t("noPurchaseHistory")}</p>
                    <p className="text-sm text-muted mt-1">{t("noPurchaseHistoryDescription")}</p>
                  </div>
                ) : (
              ...
              {completedPurchases.length > 0 && (
                <div className="flex items-center justify-between px-5 py-3 border-t border-white/[0.04] bg-white/[0.01]">
                  <p className="text-[11px] text-muted">
                    <span className="font-semibold text-foreground">{completedPurchases.length}</span> {completedPurchases.length !== 1 ? t("completedPurchasesPlural") : t("completedPurchaseSingular")}
                  </p>
                  <span className="text-[11px] text-muted">
                    {t("totalSpentLabel")}:{" "}
                    <span className="font-semibold text-foreground">
      ...
      <AnimatePresence>
        {detailOrder && (
          <OrderDetailModal
            ...
            onCancel={(id) => { handleCancelOrder(id); setDetailOrder(null); }}
            onConfirm={(id) => { handleConfirmDelivery(id); setDetailOrder(null); }}
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <div>
            <h3 className="text-base font-bold text-foreground">{t("orderDetails")}</h3>
            <p className="text-xs text-muted mt-0.5">{String(o.order_number)}</p>
          </div>
          <button onClick={onClose} className="p-2 text-muted hover:text-foreground rounded-lg hover:bg-surface transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="px-6 py-5 space-y-5 max-h-[70vh] overflow-y-auto">
          ...
          <div className="flex items-center gap-3">
            <div className="w-14 h-14 rounded-xl bg-surface border border-border flex items-center justify-center text-muted/30 shrink-0 overflow-hidden relative">
              {o.product_image ? (
                <Image src={String(o.product_image)} alt={String(o.product_name)} fill className="object-cover" sizes="56px" />
              ) : (
                <Package size={20} />
              )}
            </div>
            <div>
              <p className="text-sm font-semibold text-foreground">{String(o.product_name)}</p>
              <div className="flex items-center gap-1.5 mt-0.5">
                {o.size ? <span className="text-[11px] text-muted bg-surface px-1.5 py-0.5 rounded">{String(o.size)}</span> : null}
                <Badge variant={config.variant}>
          ...
          <div className="bg-surface/60 rounded-xl border border-border/60 p-4">
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs font-bold text-foreground uppercase tracking-wider">{t("payment")}</span>
            </div>
            <div className="space-y-2 text-sm">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-foreground font-medium">{formatPrice(taxAmount)}</span>
              </div>
          ...
          {o.shipping_name ? (
            <div className="bg-surface/60 rounded-xl border border-border/60 p-4">
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold text-foreground uppercase tracking-wider">{t("shippingAddress")}</span>
              </div>
              <div className="text-sm text-foreground space-y-0.5">
                <p className="font-medium">{String(o.shipping_name)}</p>
                <p className="text-muted">{String(o.shipping_address_line1)}</p>
                {o.shipping_address_line2 ? <p className="text-muted">{String(o.shipping_address_line2)}</p> : null}
                <p className="text-muted">
                  {String(o.shipping_city)}, {String(o.shipping_state)} {String(o.shipping_zip)}
                </p>
              </div>
            </div>
          ) : null}
            ...
            <div className="bg-surface/60 rounded-xl border border-border/60 p-4">
              ...
              {o.shipped_at ? (
                <p className="text-[11px] text-muted mt-1">
                  {t("shippedOn", { date: new Date(String(o.shipped_at)).toLocaleDateString() })}
                </p>
              ) : null}
            </div>
          ) : null}
          ...
          <div className="text-[11px] text-muted space-y-1">
            <p>{t("ordered")} {new Date(String(o.created_at)).toLocaleString()}</p>
            {o.shipped_at ? <p>{t("shipped")} {new Date(String(o.shipped_at)).toLocaleString()}</p> : null}
            {o.delivered_at ? <p>{t("delivered")} {new Date(String(o.delivered_at)).toLocaleString()}</p> : null}
          </div>
        ...
        <div className="px-6 py-4 border-t border-border flex items-center gap-3">
          ...
          {status === "shipped" && (
            ...
            >
              ...
              {confirming === orderId ? t("confirmingDelivery") : t("confirmDelivery")}
            </button>
          )}
        </div>
      </motion.div>
