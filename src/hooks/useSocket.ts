...
export function useSocket(token: string | null) {
  ...
  useEffect(() => {
    ...
    // Read receipts
    socket.on("read:update", ({ conversationId, userId }: { conversationId: string; userId: string }) => {
      store.onReadUpdate(conversationId, userId);
    });

    // Translation updates (arrives after message, updates translations field)
    socket.on("message:translated", ({ messageId, conversationId, translations }: { messageId: string; conversationId: string; translations: Record<string, string> }) => {
      store.onMessageTranslated(messageId, conversationId, translations);
    });

    // Offer updates
    socket.on("offer:update", ({ offerId, status }: { offerId: string; status: string }) => {
      store.onOfferUpdate(offerId, status);
    });

    // Unread counts
    socket.on("unread:update", ({ counts, total }: { counts: Record<string, string>; total: number }) => {
      store.onUnreadUpdate(counts, total);
    });

    // Order notifications
    socket.on("order:shipped", (data: { orderId: string; orderNumber: string; productName: string; trackingNumber: string; carrier: string }) => {
      // Show notification
